...
**Milestone 1 implemented.** The single-note retention loop
(`note -> HFE score -> gate -> ATP packet`) runs end-to-end against the real
`axis-hfe` engine (`hypothesis_field`) via a Node↔Python subprocess bridge.

```bash
# offline / no API cost (mock engine):
node bin/intentloop.js --file notes/01-cache-across-sessions.md
npm test            # gate + schema + ledger checks
npm run calibrate   # HFE score vs human-judgement table over notes/

# real run:
node bin/intentloop.js --file notes/01-cache-across-sessions.md --provider anthropic
```

What works today: 6-axis HFE scoring, threshold gate reading
`reactor/loop-policy.json`, schema-validated ATPs in `packets/`, cross-session
continuity (next run reads prior packets), and a hash-chained `ledger/`.
See [docs/BUILD_BRIEF.md](docs/BUILD_BRIEF.md) for the spec and DoD.

> Note: in `mock` mode the engine returns content-independent vectors, so the
> gate can't discriminate good vs bad notes — real calibration needs
> `--provider anthropic`. The gate/schema *logic* is proven by `npm test`.
